function [fwx,fwz,twy,data] = DEM2DwallForce(x,z,vx,vz,par,data)
    ...
    for i = 1 : par.N
        ...
        if ((x(i)< box(1)+radius(i) && x(i) > box(1)))
           ...
           normalDissipative_l = 2*par.dampN*sqrt(0.5*data.mass(i)*normalStiffness)*ddeltaW(1,i);
           ...
           if(abs(tangentialSpring) < 100*eps)
               ...
               fwz_l(i) = (tangentialStiffness*tangentialSpring - 2*par.dampTwall*sqrt(0.5*data.mass(i)*tangentialStiffness)*vz(i));
               % friction
...
%               data.contactsWall.contactPoint(i,:,2) = DEM2Drotation(data.angular(2,i)*par.dt)*data.contactsWall.contactPoint(i,:,2)'; % rotate contact point
            ...
            normalDissipative_r = -2*par.dampN*sqrt(0.5*data.mass(i)*normalStiffness)*ddeltaW(2,i);
            ...
            if(abs(tangentialSpring) < 10*eps)
                ...
                fwz_r(i) = (tangentialStiffness*tangentialSpring + 2*par.dampTwall*sqrt(0.5*data.mass(i)*tangentialStiffness)*vz(i));
            ...
            if(par.considerRotations)
                ...
                if(abs(data.contactsWall.accumulatedRollingDeformation(i,1,2)) > abs(fwx_r(i))*par.muWall/tangentialStiffness*par.Cr) %4.30
                    data.contactsWall.accumulatedRollingDeformation(i,1,2) =sign(data.contactsWall.accumulatedRollingDeformation(i,1,2))*abs(fwx_r(i))*par.muWall/tangentialStiffness*par.Cr; %data.contactsWall.accumulatedRollingDeformation(i,1,2)/abs(data.contactsWall.accumulatedRollingDeformation(i,1,2))*
        ...
        if ((z(i)<box(3)+radius(i) && z(i)>box(3)) && ( x(i)< box(3) || x(i)>box(1) ))
            ...
            ddeltaW(3,i) = -vz(i);
            fwz_b(i) = normalStiffness*deltaW(3,i) + 2*par.dampN*sqrt(0.5*data.mass(i)*normalStiffness)*ddeltaW(3,i);
            ...
            if(abs(tangentialSpring) < 10*eps)
                ...
                fwx_b(i) = -tangentialStiffness*tangentialSpring +2*par.dampTwall*sqrt(0.5*data.mass(i)*tangentialStiffness)*vx(i);
                % friction
            ...
            if(par.considerRotations)
                ...
                if(abs(data.contactsWall.accumulatedRollingDeformation(i,1,3)) > abs(fwz_b(i))*par.muWall/tangentialStiffness*par.Cr) %4.30
                    data.contactsWall.accumulatedRollingDeformation(i,1,3) =sign(data.contactsWall.accumulatedRollingDeformation(i,1,3))*(fwz_b(i))*par.muWall/tangentialStiffness*par.Cr; %data.contactsWall.accumulatedRollingDeformation(i,1,3)/abs(data.contactsWall.accumulatedRollingDeformation(i,1,3))*
...
%                  disp(["twy(i,3) no rolling resistance",twy(i,3)])
                ...
                % twy(i,3) = twy(i,3) - tangentialStiffness*data.contactsWall.accumulatedRollingDeformation(i,1,3)*norm(data.contactsWall.actuationPoint(i,:,3)'-data.position(:,i)); % projection into tangential plane necessary
%                  disp(["twy(i,3) with rolling resistance",twy(i,3)])
        ...
        if ((z(i)>box(4)-radius(i) && z(i)<box(4)) && ( x(i)< box(3) || x(i)>box(1) ))
            ...
            ddeltaW(4,i) = vz(i);
            fwz_t(i) = -normalStiffness*deltaW(4,i) - 2*par.dampN*sqrt(0.5*data.mass(i)*normalStiffness)*ddeltaW(4,i);%
            ...
            if(abs(tangentialSpring) < 10*eps)
                ...
                fwx_t(i) = tangentialStiffness*tangentialSpring - 2*par.dampTwall*sqrt(0.5*data.mass(i)*tangentialStiffness)*vx(i);
            end
