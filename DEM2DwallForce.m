    ...
    normal_stiffness = par.Emodul*pi/2*radius(i);
        ...
        if ((x(i)< box(1)+radius(i) && x(i) > box(1)))
           if(data.contactsWall.isInitialized(i,1))
            ...
            ddeltaW(1,i) = -vx(i);
            fwx_l(i) = normal_stiffness*deltaW(1,i) + 2*par.dampN*sqrt(data.mass(i)*normal_stiffness)*ddeltaW(1,i);
            ...
            if(abs(tangentialSpring) < 10*eps)
                ...
                fwz_l(i) = par.kT*tangentialSpring + 2*par.dampT*sqrt(data.mass(i)*par.kT)*vz(i);
                % friction
                if(abs(fwz_l(i)) > abs(fwx_l(i))*par.mu)
                    fwz_l(i) = sign(fwz_l(i))*abs(fwx_l(i))*par.mu;
        ...
        if ((x(i)> box(2)-radius(i) && x(i) < box(2)))
            ...
            ddeltaW(2,i) = vx(i);
            fwx_r(i) = - normal_stiffness*deltaW(2,i) - 2*par.dampN*sqrt(data.mass(i)*normal_stiffness)*ddeltaW(2,i);
            ...
            if(abs(tangentialSpring) < 10*eps)
                ...
                fwz_r(i) = par.kT*tangentialSpring + 2*par.dampT*sqrt(data.mass(i)*par.kT)*vz(i);

                % friction
                if(abs(fwz_r(i)) > abs(fwx_r(i))*par.mu)
                    fwz_r(i) = sign(fwz_r(i))*abs(fwx_r(i))*par.mu;
        ...
        if ((z(i)<box(3)+radius(i) && z(i)>box(3)) && ( x(i)< box(3) || x(i)>box(1) ))
            ...
            if(abs(tangentialSpring) < eps)
                ...
                fwx_b(i) = par.kT*tangentialSpring + 2*par.dampT*sqrt(data.mass(i)*par.kT)*vx(i);

                % friction
                if(abs(fwx_b(i)) > abs(fwz_b(i))*par.mu)
                    fwx_b(i) = sign(fwx_b(i))*abs(fwz_b(i))*par.mu;
                    data.contactsWall.actuationPoint(i,:,3) = [x(i) box(3)];
                end
            end
        ...
        if ((z(i)>box(4)-radius(i) && z(i)<box(4)) && ( x(i)< box(3) || x(i)>box(1) ))
            ...
            if(abs(tangentialSpring) < eps)
                ...
                fwx_t(i) = par.kT*tangentialSpring - 2*par.dampT*sqrt(data.mass(i)*par.kT)*vx(i);
            end
            
            % friction            
            if(abs(fwx_t(i)) > abs(fwz_t(i))*par.mu)
                fwx_t(i) = sign(fwx_t(i))*abs(fwz_t(i))*par.mu;
                data.contactsWall.actuationPoint(i,:,4) = [x(i) box(4)];
            end
