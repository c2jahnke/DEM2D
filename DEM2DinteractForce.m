...
for j = 1:length(c.contacts)
    ...
    if(data.delta(i,k) <0)
        ...
        relVel_ij = data.velocity(:,i) - data.velocity(:,k);
        %%%%%%%%%%%%%%%%%%% Coulomb Friction %%%%%%%%%%%%%%%%%%%%%%%
...
%         FnormalDiss = -par.dampN*2*sqrt(normalStiffness*effMass)*  norm(relVel_ij.*[nx; nz]);
        F(i,k) = normalStiffness*delta(i,k) - par.dampN*2*sqrt(normalStiffness*effMass)*(relVel_ij'*[nx; nz]) + Fcohesion ;%ddeltadt(i,k); 
        F(k,i) = - F(i,k);
...
%             Ft(i,k) = -tangentialSpring*tangentialStiffness;
            ...
            if(par.considerRotations)
                ...
                torqY(k,i) = Ft(i,k)*(data.contactsParticle.actuationPoint(:,k,i)- [x(k); z(k)])'*[nx; nz];
