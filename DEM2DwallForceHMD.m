function [fwx,fwz,twy,data] = DEM2DwallForceHMD(par,data,c)
    fwx = zeros(par.N,1);
    fwz = zeros(par.N,1);
...
%            ddeltaW = -data.velocity(1,i);
           ...
           %ddeltaW(1,i) = -vx(i);
           Rstar = data.radius(i);
           normalStiffness = 2*Estar*sqrt(Rstar*deltaW(k,i));
           
           normalConservative_l = 4/3*Estar*sqrt(Rstar)*deltaW(k,i)^(3/2);
           %par.dampN*2*sqrt(0.5*data.mass(i)*normalStiffness)*data.velocity(:,i)'*normal;
           normalDissipative_l = 2*sqrt(5/6)*beta*sqrt(normalStiffness*data.mass(i))*(data.velocity(:,i)'*normal);
